import { getFileName, truncate } from "./toolUtils";
...
const extractToolResultText = (result?: string | null): string | undefined => {
  if (!result) {
    return undefined;
  }
...
export const resolveToolTarget = (input: ToolInput, name?: string): ToolTargetInfo | undefined => {
  const normalizedInput = (normalizeToolInput(name, input) ?? input) as ToolInput;
