 ...
 * @param t - translation function (supports key + options interpolation)
 * @returns A function that localizes message text
 *
 * Adapted for ccagent's lightweight i18n (t accepts key + options object).
 */
type TranslateFn = (key: string, options?: Record<string, unknown>) => string;

export function createLocalizeMessage(t: TranslateFn): (text: string) => string {
