 ...
 * Ported verbatim from ccgui (pure logic, no external deps).
 */
...
export function extractPatchFromResponseItemPayload(payload) {
  ...
  if (name !== 'exec_command' || typeof payload.arguments !== 'string') return '';
...
export function parseApplyPatchToOperations(patchText) {
  ...
  const flushUpdate = () => {
    ...
    if (currentHunkHeader) {
      ...
      if (effectiveCount > 1) endLine = startLine + effectiveCount - 1;
    }

    operations.push({
      filePath: currentPath, kind: currentKind, oldString, newString,
      toolName: 'edit', startLine, endLine,
    });
  };
  ...
  const flushAdd = () => {
    ...
    operations.push({
      filePath: currentPath, kind: currentKind,
      oldString: '', newString: addFileLines.join('\n'), toolName: 'write'
    });
  };
  ...
  for (const rawLine of lines) {
    ...
    if (line.startsWith('*** Update File: ')) {
      flushUpdate(); flushAdd();
      currentPath = line.slice('*** Update File: '.length).trim();
      currentKind = 'update'; currentHunkHeader = null;
      oldLines = []; newLines = []; addFileLines = [];
      continue;
    }
    if (line.startsWith('*** Add File: ')) {
      flushUpdate(); flushAdd();
      currentPath = line.slice('*** Add File: '.length).trim();
      currentKind = 'add'; currentHunkHeader = null;
      oldLines = []; newLines = []; addFileLines = [];
      continue;
    }
    if (line.startsWith('*** Delete File: ')) {
      ...
      currentKind = 'delete'; currentHunkHeader = null;
      oldLines = []; newLines = []; addFileLines = [];
      continue;
    }
    ...
    if (line.startsWith('*** End Patch')) {
      flushUpdate(); flushAdd();
      currentPath = null; currentKind = null; currentHunkHeader = null;
      oldLines = []; newLines = []; addFileLines = [];
      continue;
    }

    if (!currentPath || !currentKind) continue;
    if (currentKind === 'delete') continue;

    if (currentKind === 'add') {
      if (line.startsWith('+')) addFileLines.push(line.slice(1));
      continue;
    }
