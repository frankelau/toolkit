/**
 * Tool name/input normalization + invocation tracking for Codex.
 * Ported from ccgui (services/codex/codex-tool-normalization.js), self-contained.
 */
...
export function normalizeMcpToolInput(server, tool, args) {
  ...
  if (serverName !== 'filesystem') return input;
  ...
  if (toolName === 'edit_file') {
    ...
    if (firstEdit) {
      if (typeof firstEdit.oldText === 'string') { input.old_string = firstEdit.oldText; input.oldString = firstEdit.oldText; }
      if (typeof firstEdit.newText === 'string') { input.new_string = firstEdit.newText; input.newString = firstEdit.newText; }
    }
  } else if (toolName === 'write_file' && typeof input.content === 'string') {
    input.old_string = ''; input.new_string = input.content;
    input.oldString = ''; input.newString = input.content;
  }
  return input;
}
...
export function normalizeUpdatePlanInput(input) {
  ...
  normalized.plan = plan
    .map((item) => {
      ...
      return { ...item, content, step: content, status: normalizePlanStatus(item.status) };
    })
    .filter(Boolean);
  return normalized;
}
...
export function normalizeFunctionCallTool(toolName, parsedArguments) {
  ...
  if (mcpMatch) {
    ...
    return {
      ...
      input: normalizeMcpToolInput(server, tool, isObjectRecord(parsedArguments) ? parsedArguments : {}),
    };
  }
  return {
    name: toolName,
